...
crud.get.select = async options => {
	...
	try {
		...
		const [[rows], [totalRows]] = await Promise.all([
			db.query(`select ${fieldStr} from ${table} ${joinStr} ${whereStr} order by ${table}.id desc limit ? offset ?`, [...binds, pageSize, offset]),
			db.query(`select count(*) total from ${table} ${whereStr}`, binds),
		])
		if (rows?.length) {
			return base.respSuccess({
				msg: '查询成功',
				total: totalRows[0]?.total || 0,
				data: base.formatDbRows(rows),
			})
		}
		return base.respSuccess({
			msg: '查询成功',
...
crud.get.detail = async options => {
	...
	try {
		const [rows] = await db.query(`select ${fieldStr} from ${table} ${joinStr} where ${table}.id = ?`, [query.id])
		if (rows?.length) {
			return base.respSuccess({
				msg: '查询成功',
				data: base.formatDbRows(rows)[0],
			})
		}
		return base.respSuccess({
			msg: '查询成功',
...
crud.post.insert = async options => {
	...
	try {
		const [res] = await db.query(`insert into ${table} (${fields.join(',')}) values (${fields.map(() => '?').join(',')})`, binds)
		if (res.affectedRows) {
			return base.respSuccess({
				msg: '新增成功',
				data: body.id,
...
crud.post.update = async options => {
	...
	try {
		const successIds = []
		for (const id of ids) {
			const [res] = await db.query(`update ${table} set ${updates.join(', ')} where id = ?`, [...binds, id])
			if (res.affectedRows) {
				successIds.push(id)
			}
		}
