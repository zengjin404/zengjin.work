  ...
  default: async function handler(req, res) {
    ...
    try {
      switch (operation) {
        case 'select':
          // 获取所有笔记
          const [notes] = await pool.query('SELECT * FROM note ORDER BY id DESC');
        ...
        case 'insert':
          ...
          const [newNote] = await pool.query('SELECT * FROM note WHERE id = ?', [result.insertId]);
        ...
        case 'update':
          ...
          await pool.query(
            `UPDATE note SET ${updateFields.join(', ')} WHERE id = ?`,
            updateValues
          );

          const [updatedNote] = await pool.query('SELECT * FROM note WHERE id = ?', [updateData.id]);
        ...
        case 'delete':
          ...
          const [deleteResult] = await pool.query('DELETE FROM note WHERE id = ?', [deleteId]);
